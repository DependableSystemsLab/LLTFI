...
def parseMasterYaml():
    ...
    try:
        ...
        print(
            "ERROR: Unable to find input.yaml or load the input.yaml under basedir directory"
        )
        print(basedir)
        sys.exit(-1)
    try:
        model_list = list(
            master_yaml_dict["compileOption"]["instSelMethod"][0]["customInstselector"][
                "include"
            ]
        )
    except Exception:
        print(
            "ERROR: this wrapper script is not applicable on the input.yaml under current directory. Please note this script is only applicable on input.yaml files with multiple software failure models defined."
        )
        print(basedir)
        sys.exit(-1)
...
def splitMasterYaml(master_yaml_dict, model_list):
    for model in model_list:
        ...
        slave_yaml_dict["compileOption"]["instSelMethod"][0]["customInstselector"][
            "include"
        ] = include_list
