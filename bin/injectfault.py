...
Usage: %(prog)s <fault injection executable> <the same options that you use to run the executable before>
    ...
    for index, opt in enumerate(optionlist):
        if os.path.isfile(opt):
            if os.path.realpath(os.path.dirname(opt)) != basedir:
                usage(
                    "File %s passed through option is not under current directory" % opt
                )
            else:
...
def checkInputYaml():
    ...
    try:
        ...
        usage(
            "No input.yaml file in the parent directory of fault injection executable"
        )
        sys.exit(1)
    except Exception:
        usage(
            "input.yaml is not formatted in proper YAML (reminder: use spaces, not tabs)"
        )
        sys.exit(1)
...
def storeInputFiles():
    ...
    for opt in temp_optionlist:
        if os.path.isfile(
            opt
        ):  # stores all files in inputList and copy over to inputdir
    ...
    for each in newfiles:
        if each not in dirBefore:
            ...
            if fileSize == 0 and each.startswith("llfi"):
                ...
                print(
                    each + " is going to be deleted for having size of " + str(fileSize)
                )
                os.remove(each)
            else:
                flds = each.split(".")
        ...
        while 1:
            ...
            if line.strip():
                ...
                if label == "total_cycle":
                    ...
                    fi_ml_stats.append(
                        [int(layerNum), layerName, int(cycleStart), int(cycleEnd)]
                    )
...
def checkValues(key, val, var1=None, var2=None, var3=None, var4=None):
    ...
    if key == "run_number":
        ...
        assert isinstance(val, int), key + " must be an integer in input.yaml"
        assert int(val) >= 0, (
            key + " must be greater than or equal to zero in input.yaml"
        )

    elif key == "fi_max_multiple":
        assert isinstance(val, int), key + " must be an integer in input.yaml"
        assert int(val) > 0, key + " must be greater than zero in input.yaml"
        assert int(val) <= int(fi_max_multiple_default), (
            key
            + " must be smaller than or equal to "
            + str(fi_max_multiple_default)
            + " in input.yaml"
        )
        ...
        assert isinstance(val, int), key + " must be an integer in input.yaml"
        assert int(val) > 0, key + " must be greater than 0 in input.yaml"
        assert int(val) <= int(totalcycles), (
            key
    ...
    if not os.path.isfile(fi_exe):
        ...
        for ii, run in enumerate(rOpt):
            ...
            if "fi_type" in run["run"]:
                ...
                if (
                    ...
                    try:
                        ...
                        injectorname = cOpt["instSelMethod"][0]["customInstselector"][
                            "include"
                        ][0]
                    except Exception:
                        print(
            ...
            if "window_len_multiple_startindex" in run["run"]:
                window_len_multiple_startindex = run["run"][
                    "window_len_multiple_startindex"
                ]
                checkValues(
                    "window_len_multiple_startindex", window_len_multiple_startindex
                )
                if "window_len_multiple_startindex" in locals():
                    if "window_len" in run["run"]:
                        print(
            ...
            if "window_len_multiple_endindex" in run["run"]:
                window_len_multiple_endindex = run["run"][
                    "window_len_multiple_endindex"
                ]
                checkValues(
                    "window_len_multiple_endindex", window_len_multiple_endindex
                )
                if "window_len_multiple_endindex" in locals():
                    if "window_len" in run["run"]:
                        print(
            ...
            if "fi_bit" in run["run"]:
                ...
                checkValues(
                    "fi_bit", fi_bit, run_number, fi_cycle, fi_index, fi_reg_index
                )
            ...
            if (
                "window_len_multiple" in locals()
                or "window_len_multiple_startindex" in locals()
                or "window_len_multiple_endindex" in locals()
            ):
                if "fi_max_multiple" not in locals():
                    print(
                        (
                            ...
                            " Thus, the maximum number of locations will be chosen as "
                            + str(fi_max_multiple_default)
                            + ".\n"
                        )
                    )
            ...
            if (
                "window_len_multiple" not in locals()
                and "window_len_multiple_startindex" not in locals()
            ) and "fi_max_multiple" in locals():
                print(
                    (
                ...
                with open("llfi.config.runtime.txt", "w") as ficonfig_File:
                    if "fi_cycle" in locals() and len(fi_ml_stats) > 0:
                        ...
                        for i in range(0, len(fi_ml_stats)):
                            if (
                                fi_cycle >= fi_ml_stats[i][2]
                                and fi_cycle <= fi_ml_stats[i][3]
                            ):
                                ficonfig_File.write(
                                    "ml_layer_name=" + fi_ml_stats[i][1] + "\n"
                                )
                                ficonfig_File.write(
                                    "ml_layer_number=" + str(fi_ml_stats[i][0]) + "\n"
                                )
                    ...
                    if "window_len" in locals():
                        fi_second_cycle = min(
                            fi_cycle + random.randint(1, int(window_len)),
                            int(totalcycles),
                        )
                        ficonfig_File.write(
                            "fi_second_cycle=" + str(fi_second_cycle) + "\n"
                        )
                    if "fi_max_multiple" in locals():
                        win_start_index = 1
                        win_end_index = 1
                        if "window_len_multiple" in locals():
                            ...
                            "window_len_multiple_startindex" in locals()
                            and "window_len_multiple_endindex" in locals()
                        ):
                            ...
                            if win_start_index > win_end_index:
                                ...
                                sys.exit(1)
                        ficonfig_File.write(
                            "fi_max_multiple=" + str(fi_max_multiple) + "\n"
                        )
                        ...
                        for _ in range(1, int(selected_num_of_injection)):
                            fi_next_cycle = min(
                                fi_next_cycle
                                + random.randint(win_start_index, win_end_index),
                                int(totalcycles),
                            )
                            ficonfig_File.write(
                                "fi_next_cycle=" + str(fi_next_cycle) + "\n"
                            )
