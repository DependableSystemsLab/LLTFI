    ...
    while argid < len(args):
        ...
        if arg.startswith("-"):
            if arg == "--dir":
                ...
                argid += 1
                options["L"].append(
                    os.path.abspath(os.path.join(os.getcwd(), args[argid]))
                )
...
def checkInputYaml():
    ...
    try:
        ...
        print(
            "Error: input.yaml is not formatted in proper YAML (reminder: use spaces, not tabs)"
        )
        os.rmdir(options["dir"])
        sys.exit(1)
            ...
            if attr == "include" or attr == "exclude":
                ...
                if methodName == "insttype":
                    ...
                    if custom_instselector_defined:
                        ...
                        print(
                            "To apply a list of fault models/failure modes, please use 'batchinstrument'"
                        )
                        sys.exit(1)
                    else:
        ...
        # Select by custom register
            ...
            if "customRegSelector" not in cOpt:
                ...
                if (
                    ...
                    try:
                        regselectorname = cOpt["instSelMethod"][0][
                            "customInstselector"
                        ]["include"][0]
                    except Exception:
                        print(
                    ...
                    compileOptions.append(
                        "-firegselectorname=" + cOpt["customRegSelector"]
                    )
    ...
    if "tracingPropagation" in cOpt and cOpt["tracingPropagation"]:
        ...
        if "tracingPropagationOption" in cOpt:
            if "debugTrace" in cOpt["tracingPropagationOption"]:
                if (
                    str(cOpt["tracingPropagationOption"]["debugTrace"]).lower()
                    == "true"
                ):
                    compileOptions.append("-debugtrace")
            if "maxTrace" in cOpt["tracingPropagationOption"]:
                assert isinstance(
                    cOpt["tracingPropagationOption"]["maxTrace"], int
                ), "maxTrace must be an integer in input.yaml"
                assert (
                    int(cOpt["tracingPropagationOption"]["maxTrace"]) > 0
                ), "maxTrace must be greater than 0 in input.yaml"
            ...
            if "generateCDFG" in cOpt["tracingPropagationOption"]:
                if (
                    str(cOpt["tracingPropagationOption"]["generateCDFG"]).lower()
                    == "true"
                ):
    ...
    if not options["IRonly"]:
        ...
        if retcode == 0:
            ...
            if retcode != 0:
                print(
                    ...
                    + os.path.basename(llvmgcc)
                    + ", trying with "
                    + os.path.basename(llvmgxx)
                    + "."
                )
                execlist[0] = llvmgxx
                retcode = execCompilation(execlist)
        ...
        if retcode != 0:
            print(
                ...
                "in directory %s)."
                % (proffile + _suffixOfIR(), fifile + _suffixOfIR(), llfilinklib),
                file=sys.stderr,
            )
            sys.exit(retcode)
        else:
