...
public:
  ...
  bool runOnModule(Module& M) override {
    ...
    for (std::set<std::string>::iterator name =
             all_hardware_inst_selector_names.begin();
    ...
    for (std::set<std::string>::iterator name =
             all_hardware_reg_selector_names.begin();
...
static RegisterPass<HardwareFailureAutoScanPass>
    X("HardwareFailureAutoScanPass",
      "Automatic scanner of hardware failure modes (instruction selectors, reg "
      "selectors)",
      false, false);
} // namespace llfi
