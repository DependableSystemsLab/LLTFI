...
public:
  ...
  bool runOnModule(Module& M) override {
    ...
    for (std::set<std::string>::iterator name =
             all_software_failure_names.begin();
      ...
      for (std::map<Instruction*, std::list<int>*>::iterator MI =
...
static RegisterPass<SoftwareFailureAutoScanPass>
    X("SoftwareFailureAutoScanPass",
      "Automatic scanner of software failure modes", false, false);
} // namespace llfi

// Free function callable from RegisterPasses.cpp for the new PM wrapper.
namespace llfi {
void runSoftwareFailureAutoScan(llvm::Module& M) {
  SoftwareFailureAutoScanPass().runOnModule(M);
}
} // namespace llfi