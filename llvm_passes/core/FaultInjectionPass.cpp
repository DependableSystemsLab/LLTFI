...
void FaultInjectionPass::insertInjectionFuncCall(
  ...
  for (std::map<Instruction *, std::list<int> *>::iterator inst_reg_it =
           inst_regs_map->begin();
       inst_reg_it != inst_regs_map->end(); ++inst_reg_it) {
    ...
    for (std::list<int>::iterator reg_pos_it = fi_reg_pos_list->begin();
      ...
      // Turn off tail call optimization, i.e., fastcc calling convention
      ...
      if (*reg_pos_it == DST_REG_POS)
        fi_reg = fi_inst;
      else
        fi_reg = fi_inst->getOperand(*reg_pos_it);
      ...
      Value *injectfunc = M.getOrInsertFunction(funcname, injectfunctype);
      ...
      args[2] =
          ConstantInt::get(i32type, fi_inst->getOpcode()); // opcode in i32
      args[3] = ConstantInt::get(i32type, reg_index); // reg_index not reg_pos
      args[4] = ConstantInt::get(i32type, total_reg_num); // total_reg_num
      //======== Add reg_pos QINING @DEC 23rd ==========
      args[5] = ConstantInt::get(
          i32type, *reg_pos_it + 1); // dstreg->0, operand0->1, operand1->2 ...
      //================================================
      //======== Add opcode_str QINING @MAR 11th========
	...
	/*errs() << "\nType: ";
        opcode_str_gv->getType()->getContainedType(0u)->print(errs());
    
        printf("\n Constant: %s\n",PrintValue((Constant* &)opcode_str_gv).c_str());


	errs() << "\nPointerType: ";
	cast<PointerType>(opcode_str_gv->getType()->getScalarType())->print(errs());

	errs() << "\nContainedType: ";
	cast<PointerType>(opcode_str_gv->getType()->getScalarType())->getContainedType(0u)->print(errs());

	errs() << "\n\n";

	for (ArrayRef<Constant*>::iterator const_it = indices_for_gep_array_ref.begin();
         const_it != indices_for_gep_array_ref.end(); ++const_it) {	
		(*const_it)->print(errs());
		errs() << "\n";
	}
	...
	errs() << "\n\n\n";*/
      ...
      Constant *gep_expr = ConstantExpr::getGetElementPtr(opcode_str_gv->getType()->getContainedType(0u), (Constant* &)opcode_str_gv, indices_for_gep_array_ref, true);

     
	

	//printf("gep_expr is: %s\n",PrintValue(gep_expr).c_str());
...
void FaultInjectionPass::createInjectionFuncforType(Module &M, Type *fitype,
                                                    std::string &fi_name,
                                                    Constant *injectfunc,
...
Constant *FaultInjectionPass::getLLFILibFIFunc(Module &M) {
  ...
  fi_func_param_types[2] =
      PointerType::get(Type::getInt8Ty(context), 0);  // inst
...
static RegisterPass<FaultInjectionPass> X("faultinjectionpass",
                                          "Fault injection pass", false, true);
}
