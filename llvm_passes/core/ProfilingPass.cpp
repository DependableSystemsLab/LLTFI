...
void insertCallForMLFIStats(Module& M) {
  ...
  for (Function::iterator bb = main_graph->begin(); bb != main_graph->end();
       bb++) {
    ...
    if (isa<CallInst>(fi_inst)) {
      ...
      for (std::list<int>::iterator reg_pos_it_mem = fi_regs->begin();
        ...
        if ((reg_mem.find("memcpy") != std::string::npos) ||
          ...
          logFile << "LLFI cannot instrument " << reg_mem
                  << " intrinsic function" << "\n";
          continue_flag = true;
          break;
        }
...
static RegisterPass<LegacyProfilingPass> X("profilingpass", "Profiling pass",
                                           false, false);
} // namespace llfi
