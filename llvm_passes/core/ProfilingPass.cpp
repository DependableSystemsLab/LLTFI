...
bool ProfilingPass::runOnModule(Module &M) {
  ...
  for (std::map<Instruction *, std::list<int> *>::const_iterator inst_reg_it =
    ...
    CallInst::Create(profilingfunc, profilingarg_array_ref, "", insertptr);
  }
...
Constant *ProfilingPass::getLLFILibEndProfilingFunc(Module &M) {
  ...
  Constant *endprofilefunc =
      M.getOrInsertFunction("endProfiling", endprofilingfunctype);
  return endprofilefunc;
}

static RegisterPass<ProfilingPass> X("profilingpass", "Profiling pass", false,
                                     false);
}
