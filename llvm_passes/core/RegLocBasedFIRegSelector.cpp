...
bool RegLocBasedFIRegSelector::isRegofInstFITarget(Value *reg,
  ...
  if (firegloc == dstreg) {
    ...
    if (isa<GetElementPtrInst>(inst)) {
      if (inst->getOperand(inst->getNumOperands() - 1) == reg &&
          isa<Constant>(reg))
        return false;
    }
    return reg != inst;
    ...
    if (srcindex < totalsrcregnum) {
      if (isa<GetElementPtrInst>(inst)) {
        if (inst->getOperand(totalsrcregnum - 1) == reg && isa<Constant>(reg))
          return false;
      }
...
bool RegLocBasedFIRegSelector::isRegofInstFITarget(Value *reg,
  ...
  if (firegloc == allsrcreg || firegloc == dstreg)
    return isRegofInstFITarget(reg, inst);
  else
    return isRegofInstFITarget(reg, inst) && (firegloc - srcreg1) == pos;
}
}
