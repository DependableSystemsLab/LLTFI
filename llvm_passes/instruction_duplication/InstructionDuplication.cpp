...
static cl::opt<string> layerName("operatorName", cl::desc("Name of \
        ...
        Example: conv;relu;matmul;maxpool;all"),
                                 cl::init("all"));

static cl::opt<bool> enableChainDuplication(
    "enableChainDuplication",
    cl::desc("Boolean value to indicate whether to do arithmetic chain \
            duplication or not. Default: False"),
    cl::init(false));
...
static int64_t getOperatorNumber(string name) {
  ...
  map<string, int64_t> ONNXOperatorId = {{"conv", 1986948931},
                                         {"relu", 1970038098},
...
private:
  ...
  void initializeGranularityAndLayerName(string llfiIndex, string layerName,
                                         bool isChainDupl) {
...
public:
  ...
  void duplicateInstructionChain(vector<Instruction*> insVector) {
    ...
    assert(lastInst != nullptr && lastInstDupl != nullptr);
    IRBuilder<> IRB(lastInst->getParent());
    IRB.SetInsertPoint(lastInstDupl->getNextNode());

    auto Fn = lastInst->getFunction()->getParent()->getOrInsertFunction(
        "compareFloatValues", Type::getFloatTy(lastInst->getContext()),
  ...
  bool runOnMainGraph(Function& F) {
    ...
    if (!isInitialized) {
      isInitialized = true;
      initializeGranularityAndLayerName(llfiIndex.getValue(),
                                        layerName.getValue(),
                                        (bool)enableChainDuplication);
    }
