...
void parseLLTFIConfigFile() {
  ...
  while (fgets(line, CONFIG_LINE_LENGTH, ficonfigFile) != nullptr) {
    ...
    // Parse FI stats for ML applications
      ...
      if (LLTFI_config.fi_ml_layer_name[strlen(LLTFI_config.fi_ml_layer_name) -
                                        1] == '\n')
        LLTFI_config
            .fi_ml_layer_name[strlen(LLTFI_config.fi_ml_layer_name) - 1] = '\0';
    }
  ...
  assert(LLTFI_config.fi_max_multiple > 0 &&
         "invalid fi_max_multiple in config file");
  assert((LLTFI_config.fi_ml_layer_num > 0 ||
          LLTFI_config.fi_ml_layer_num == -1) &&
...
void injectFunc(long llfi_index, unsigned size, char *buf,
  ...
  if (LLTFI_config.fi_ml_layer_num > 0)
    fprintf(injectedfaultsFile,
            ...
            LLTFI_config.fi_type, LLTFI_config.fi_max_multiple, llfi_index,
            LLTFI_CurrentCycle, my_reg_index, reg_pos, size, fi_bitpos,
            opcode_str, oldVal.ui, newVal.ui, oldVal.f, newVal.f,
