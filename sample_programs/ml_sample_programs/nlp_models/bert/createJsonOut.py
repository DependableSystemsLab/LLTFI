...
def main(inpSample):
    ...
    eval_examples = run_squad.read_squad_examples(
        input_file=os.path.join(SQUAD_DIR, "dev-v1.1.json"), is_training=False
    )
    eval_writer = run_squad.FeatureWriter(
        filename=os.path.join(OUT, "eval.tf_record"), is_training=False
    )
    eval_features = []
    ...
    for index in range(len(listResArr)):
        ...
        all_results.append(
            RawResult(
                unique_id=uniqueId,
                start_logits=listResArr[index][1],
                end_logits=listResArr[index][0],
            )
        )
        ...
        shutil.move(
            os.path.join(OUT, out_predictions_json),
            os.path.join(ONNX_PRED, out_predictions_json),
        )
        shutil.move(
            os.path.join(OUT, out_nbestpredictions_json),
            os.path.join(ONNX_NBEST_PRED, out_nbestpredictions_json),
        )
