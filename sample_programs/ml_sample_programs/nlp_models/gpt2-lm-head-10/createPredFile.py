...
inputs.append(
    "US-led coalition air strikes on a jail run by the Islamic State group in eastern Syria killed"
)
inputs.append(
    "A magazine supplement with an image of Adolf Hitler and the title 'The Unreadable Book' is pictured in"
)
inputs.append(
    "Winter isn't done with us yet. Ottawa can expect another 10 to 15 centimetres of"
)
inputs.append(
    "Refined mansion tax proposal being fed into debate on abolishing 50p tax rate for those earning more than"
)
inputs.append(
    "Ghazala Khan, the mother of a fallen U.S. soldier of Muslim faith, is responding to Donald Trump’s"
)
...
def main(inpSample):
    ...
    for elemIndex in range(len(list_output_np)):
        input_to_model = tf.convert_to_tensor(
            [[tokenizer.encode(inputs[inpSample], add_special_tokens=True)]]
        )  # Shape [1, 1, len]
