...
prompts = []
prompts.append(
    "translate English to German: I declare resumed the session of the European Parliament."
)
...
prompts.append(
    "translate English to French: I declare resumed the session of the European Parliament."
)
prompts.append(
    "translate English to German: Ladies and gentlemen, on Saturday, as you know, an earthquake"
)
prompts.append(
    "translate English to French: Ladies and gentlemen, on Saturday, as you know, an earthquake"
)
prompts.append(
...
prompts.append(
    "translate English to German: I should like, on behalf of the European Parliament, to express"
)
prompts.append(
    "translate English to French: I should like, on behalf of the European Parliament, to express"
)
...
class GenerativeT5_pytorch(torch.nn.Module):
    ...
    def __init__(
        self, encoder, decoder_with_lm_head, tokenizer, onnx=False, cuda=False
    ):
        super().__init__()
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            new_logits = []
            generated = torch.tensor(self.tokenizer(prompt)["input_ids"])[
            ...
            if self.onnx:
                encoder_outputs_prompt = self.encoder.run(
                    None, {"input_ids": generated.cpu().numpy()}
                )[0]
            else:
                    ...
                    outputs = torch.tensor(
                        self.decoder_with_lm_head.run(
                            None,
                            {
                                ...
                                "encoder_hidden_states": encoder_outputs_prompt,
                            },
                        )[0][0]
                    )
                else:
                    outputs = self.decoder_with_lm_head(
                        input_ids=generated,
                        encoder_hidden_states=encoder_outputs_prompt,
                    )[0]
                print(generated)
...
def createInp(inpSample):
    ...
    with torch.no_grad():
        generated = torch.tensor(tokenizer(prompts[inpSample])["input_ids"])[
            : max_context_length - 1
        ].unsqueeze(0)
        encoder_hidden_state = encoder_sess.run(
            None, {"input_ids": generated.cpu().numpy()}
        )[0]
...
def main():
    ...
    for i in range(0, 10):
        ...
        encoder_sess = InferenceSession("t5-encoder-12.onnx")
        _, _, tokenizer = get_encoder_decoder_tokenizer()
        generative_t5 = GenerativeT5_pytorch(
            encoder_sess, decoder_sess, tokenizer, onnx=True
        )
        token, logit = generative_t5(prompts[i], 16, temperature=0.0)
        # pdb.set_trace()
        createInp(i)
