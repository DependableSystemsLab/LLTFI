...
paths = list(
    paths.list_images(os.getcwd() + "/data")
)  # collect the image ids (name) to send datagen.py
ids = []
for i in paths:
...
partition = {
    "train": ids[: int(len(ids) * 0.8)],
    "validation": ids[-int(len(ids) * 0.2) :],
}
labels = {}
...
params = {"dim": (66, 200, 3), "batch_size": 2, "shuffle": True}
...
model.fit_generator(
    generator=training_generator, epochs=10, validation_data=validation_generator
)
