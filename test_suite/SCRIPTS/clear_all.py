...
def clear_all():
    ...
    for test in suite["HardwareFaults"]:
        fs = [
            f for f in os.listdir(os.path.join(testsuite_dir, "HardwareFaults", test))
        ]
        for f in fs:
            if f != "input.yaml":
                print("MSG: Removing ", "HardwareFaults/" + test + "/" + f)
                if os.path.isdir(
                    os.path.join(testsuite_dir, "HardwareFaults", test, f)
                ):
                    shutil.rmtree(
                        os.path.join(testsuite_dir, "HardwareFaults", test, f)
                    )
    ...
    for test in suite.get("SoftwareFaults", {}):
        fs = [
            f for f in os.listdir(os.path.join(testsuite_dir, "SoftwareFaults", test))
        ]
        for f in fs:
            if f != "input.yaml":
                print("MSG: Removing ", "SoftwareFaults/" + test + "/" + f)
                if os.path.isdir(
                    os.path.join(testsuite_dir, "SoftwareFaults", test, f)
                ):
                    shutil.rmtree(
                        os.path.join(testsuite_dir, "SoftwareFaults", test, f)
                    )
