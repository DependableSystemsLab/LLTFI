...
def startEchoServer(work_dir):
    ...
    server = subprocess.Popen(
        execlist, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
    q = Queue()
...
def callLLFI(work_dir, target_IR, prog_input):
    ...
    with open("llfi.test.log.instrument.txt", "w", buffering=1) as log:
        p = subprocess.Popen(
            [instrument_script, "--readable", "-lpthread", target_IR],
            stdout=log,
            stderr=log,
        )
        p.wait()
    ...
    with open("llfi.test.log.profile.txt", "w", buffering=1) as log:
        ...
        if target_IR == "echoClient.ll":
            try:
                ...
                print(
                    "ERROR: Unable to terminate echoServer.exe in profile for:",
                    work_dir,
                )
        if p.returncode != 0:
    ...
    with open("llfi.test.log.injectFault.txt", "w", buffering=1) as log:
        ...
        execlist.extend(prog_input.split(" "))
        p = subprocess.Popen(execlist, stdout=log, stderr=log)
        t = {"name": " ".join(work_dir.split("/")[-3:]) + "/" + target_IR, "process": p}
        if target_IR == "echoClient.ll":
            p.wait()
            try:
                server.terminate()
                ...
                print(
                    "ERROR: Unable to terminate echoServer.exe in injectfault for",
                    work_dir,
                )
