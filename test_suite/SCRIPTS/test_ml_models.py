...
def test_tensorflow_pipeline():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            result_list.append({"name": f"{prefix}/train", "result": f"FAIL: {e}"})
            return result_list
        ...
        try:
            ...
            result_list.append({"name": f"{prefix}/onnx_valid", "result": f"FAIL: {e}"})
...
def test_pytorch_pipeline():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            result_list.append(
                {"name": f"{prefix}/export_onnx", "result": f"FAIL: {e}"}
            )
            return result_list
...
def test_onnx_to_ir(onnx_path=None):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if (
            p.returncode != 0
            or not os.path.isfile(ll_out)
            or os.path.getsize(ll_out) == 0
        ):
            result_list.append(
                {
                    "name": f"{prefix}/translate_to_ll",
...
def test_fault_injection():
    ...
    mnist_dir = os.path.join(
        source_root, "sample_programs", "ml_sample_programs", "vision_models", "mnist"
    )
