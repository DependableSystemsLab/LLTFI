...
def test_ml_tools():
    ...
    if not _has_dep("onnx", "pygraphviz"):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if p.returncode != 0 or "No mismatch found" not in p.stdout:
                result_list.append(
                    {
                        "name": "./ml_tools/compare_layer_outputs_match",
                        "result": f'FAIL: expected "No mismatch found", got: {p.stdout.strip()[:120]}',
                    }
                )
            else:
                result_list.append(
                    {"name": "./ml_tools/compare_layer_outputs_match", "result": "PASS"}
                )
            ...
            if p.returncode != 0 or "No mismatch found" in p.stdout:
                result_list.append(
                    {
                        "name": "./ml_tools/compare_layer_outputs_diff",
                        "result": f"FAIL: expected mismatch detection, got: {p.stdout.strip()[:120]}",
                    }
                )
            else:
                result_list.append(
                    {"name": "./ml_tools/compare_layer_outputs_diff", "result": "PASS"}
                )
    ...
    if not _has_dep("onnx"):
        result_list.append(
            {
                "name": "./ml_tools/extend_onnx_model",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if p.returncode != 0 or not os.path.isfile(model_out):
                ...
                result_list.append(
                    {"name": "./ml_tools/extend_onnx_model", "result": "PASS"}
                )
    ...
    if not _has_dep("onnx", "pydot"):
        result_list.append(
            {
                "name": "./ml_tools/output_onnx_graph",
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if p.returncode != 0 or not os.path.isfile(dot_path):
                ...
                result_list.append(
                    {"name": "./ml_tools/output_onnx_graph", "result": "PASS"}
                )
