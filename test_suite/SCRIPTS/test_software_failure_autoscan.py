...
def test_software_failure_autoscan():
    ...
    try:
        ...
        result_list.append(
            {
                "name": "./SoftwareFailureAutoScan/scan",
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if p.returncode != 0 or not os.path.isfile(ll_file):
            result_list.append(
                {
                    "name": "./SoftwareFailureAutoScan/compile",
                    "result": f"FAIL: clang returned {p.returncode}: {p.stderr.strip()}",
                }
            )
            return 1, result_list
        result_list.append(
            {"name": "./SoftwareFailureAutoScan/compile", "result": "PASS"}
        )
        ...
        if not os.path.isfile(output_file):
            result_list.append(
                {
                    "name": "./SoftwareFailureAutoScan/output_file",
                    "result": "FAIL: llfi.applicable.software.failures.txt not generated",
                }
            )
            return 1, result_list
        result_list.append(
            {"name": "./SoftwareFailureAutoScan/output_file", "result": "PASS"}
        )
        ...
        if not content:
            result_list.append(
                {
                    "name": "./SoftwareFailureAutoScan/finds_modes",
                    "result": "FAIL: output file is empty — no failure modes found",
                }
            )
            return 1, result_list
        result_list.append(
            {"name": "./SoftwareFailureAutoScan/finds_modes", "result": "PASS"}
        )
