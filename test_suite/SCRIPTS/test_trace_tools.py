...
def callTraceTools(work_dir, resources):
    ...
    if not os.path.isfile(golden_trace_file):
    ...
    for faulty_trace in resources["trace_inject"]:
        ...
        if not os.path.isfile(faulty_trace_file):
            print(
                "WARNING: faulty_trace_file not found:",
                faulty_trace,
                "work_dir:",
                work_dir,
            )
            ...
            report_name = (
                ".".join(faulty_trace.split(".")[0:-1])
                + ".report."
                + faulty_trace.split(".")[-1]
            )
            report_file = os.path.join(work_dir, report_name)
            with open(report_file, "w") as report_out:
                p = subprocess.Popen(
                    [tracediff_script, golden_trace_file, faulty_trace_file],
                    stdout=report_out,
                )
            p.wait()
            if p.returncode != 0:
            ...
            if os.path.getsize(report_file) == 0:
                return (
                    "FAIL: report_file generated by 'tracediff' is empty:",
                    report_name,
                )
    ...
    if not os.path.isfile(report_file):
        return (
            "FAIL: united report_file not generated by 'traceunion':",
            united_report_name,
        )
    if os.path.getsize(report_file) == 0:
        return (
            "FAIL: united report_file generated by 'traceunion' is empty:",
            united_report_name,
        )

    ## call traceontograph to generate the dot file
    cdfg_prof_file = os.path.join(work_dir, resources["cdfg_prof"])
    if not os.path.isfile(cdfg_prof_file):
    ...
    with open(cdfg_faulty_file, "w") as cdfg_out:
        p = subprocess.Popen(
            [traceontograph_script, united_report_file, cdfg_prof_file], stdout=cdfg_out
        )
    p.wait()
    if p.returncode != 0:
    ...
    if not os.path.isfile(report_file):
        return (
            "FAIL: cdfg_faulty_file not generated by 'traceontograph':",
            cdfg_faulty_name,
        )
    if os.path.getsize(report_file) == 0:
        return (
            "FAIL: cdfg_faulty_file generated by 'traceontograph' is empty:",
            cdfg_faulty_name,
        )
    ...
    if len(t) < 2:
        return (
            "FAIL: dot/report files generated by'tracetodot' not complete!",
            work_dir,
        )
