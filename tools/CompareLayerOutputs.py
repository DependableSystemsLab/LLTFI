...
class DotGraph:
    ...
    def addMisMatch(self, layer, index, elements, old_val, new_val):
        ...
        self.Graph.add_node(
            old_val_node_name, label=str("Old Value: \n" + self.formatFloat(old_val))
        )
        self.Graph.add_node(
            new_val_node_name, label=str("New Value: \n" + self.formatFloat(new_val))
        )
        ...
        self.Graph.add_subgraph(
            [old_val_node_name, new_val_node_name],
            name=NameOfSubGraph,
            color="blue",
            label="< <B>Index " + str(index) + " / " + str(elements) + "</B> >",
            style="dashed",
        )
...
class LLTFI:
    ...
    def reportMismatch(self, LayerId, d1_old, d2_new):
        if not self.is_new_file:
            ...
            if str(LayerId) == str(self.fi_layer_id):
                self.fi_layer_op_mismatches[self.curr_file_name].append(
                    [d1_old, d2_new]
                )
    ...
    def getFIStats(self):
        ...
        for k, v in self.fi_stats.items():
            if len(v) == 2:
                ...
                if len(v) > 2:
                    self.multiple_output_corruption = (
                        self.multiple_output_corruption + 1
                    )
    ...
    def isMultipleBitFlips(self, d1, d2):
        ...
        numberOfFlips = (
            self.countBitFlips(sign1, sign2)
            + self.countBitFlips(exp1, exp2)
            + self.countBitFlips(man1, man2)
        )
    ...
    def printSummary(self):
        ...
        print(
            "Total mismatches found in layer "
            + str(self.fi_layer_id)
            + " = "
            + str(self.total_mismatches_found)
        )
        percentage = (float)(self.total_mismatches_found / self.total_runs) * 100
        print(
            """% of bitflips that corrupted the output of the FI layer: """
            + str(percentage)
            + "%"
        )
        percentage = (float)(
            self.single_output_corruption / self.total_mismatches_found
        ) * 100
        print(
            """% of times a single bitflip caused just a single corrpution in layer output: """
            + str(percentage)
            + "%"
        )
        print(
            "FI in bits that cuased a sign flip: "
            + str(list(set(self.fi_bit_sign_flip)))
        )
        percentage = (float)(
            self.single_bit_flips / (self.single_bit_flips + self.multiple_bit_flips)
        ) * 100
        print(
            "% of single bit corruption in the output of the tensor operator: "
            + str(percentage)
            + "%"
        )
        print(
            "Number of NaNs in output of the corrupted tensor operator: "
            + str(self.numbers_of_nan)
        )
...
def export_dot_graph(layerNames, GraphName, SelectedLayers):
    ...
    for item in mismatch:
        ...
        if (
            ("all" in SelectedLayers)
            or (layer_name.split("_")[0] in SelectedLayers)
            or (layer_name is layerNamesSmall[-1])
        ):
        ...
        with open(j2, "r") as g:
            ...
            try:
                ...
                for i in range(0, len(jf), 1):
                    ...
                    assertFun(
                        jf[key]["Number of Elements"] == jg[key]["Number of Elements"]
                    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="0",
        help="What should be the minimum difference between the mismatches? \t Default: 0.0",
    )
    parser.add_argument(
        "-f",
        "--folder",
        action="store_true",
        default=False,
        help="Specify this flag if the second argument is a folder path? \t Default: False",
    )
    parser.add_argument(
        "--dot",
        action="store_true",
        default=False,
        help="Do you want to output a dot file? \t Default: False",
    )
    parser.add_argument(
        "--nlp",
        action="store_true",
        default=False,
        help="Is the model a NLP model? \t Default: False",
    )
    parser.add_argument(
        "-fdiff",
        "--print_diff_final_op",
        action="store_true",
        default=False,
        help="Print mismatch on terminal only when there is a mismatch in the final layer o/p. \t Default: False",
    )
    parser.add_argument(
        "--selected_layers_in_dot",
        action="store",
        default="all",
        help="""Show only mismatches in selected layers during DOT file generation. Semicolon seperate values. \t Default: "all" """,
    )
    parser.add_argument(
        "--summary",
        action="store_true",
        default=False,
        help="Do you just want to output a summary of mismatches? \t Default: False",
    )
    parser.add_argument(
        "--getFIStatsRQ2",
        action="store_true",
        default=False,
        help="Should I compute FI stats? \t Default: False",
    )
    parser.add_argument(
        "--fiStatsDir",
        action="store",
        type=str,
        default="None",
        help="Directory path containg logs produced by LLTFI at runtime. \t Default: None",
    )
        ...
        if os.path.isdir(dir_path):
            ...
            if len(global_mismatch) > 0:
                ...
                if args.summary:
                    print(
                        "Mismatches found in "
                        + str(len([key for key in global_mismatch.keys()]))
                        + " file(s)."
                    )
                else:
                    print(
                        "Mismatch found in: "
                        + str([key for key in global_mismatch.keys()])
                    )
    ...
    if args.dot and (not args.folder):
        ...
        export_dot_graph(
            output_names, model.graph.doc_string, args.selected_layers_in_dot.split(";")
        )
