...
parser.add_argument(
    "--model_path", type=str, required=True, help="Path to the ONNX model"
)
parser.add_argument(
    "--output_model_path",
    type=str,
    required=True,
    help="Path to the extended ONNX model",
)
parser.add_argument(
    "--layers",
    type=str,
    default="all",
    help="""Intermediate layers for which output has to be extracted. Semicolon seperated. like: --layers="conv;maxpool" """,
)
parser.add_argument(
    "--summary",
    default=False,
    action="store_true",
    help="Just return the summary of the model",
)
...
def extend_model_output(model, intermediate_outputs):
    ...
    for output_name in intermediate_outputs:
        ...
        if (
            all_layers
            and ("Constant" not in output_name)
            and ("Gather" not in output_name)
        ):
    ...
    if (not all_layers) and ((len(intermediate_outputs) - 1) not in layer_output):
        # Add original output
        output_value_info = onnx.helper.make_tensor_value_info(
            original_op, DUMMY_TENSOR_TYPE, None
        )
        model.graph.output.extend([output_value_info])
