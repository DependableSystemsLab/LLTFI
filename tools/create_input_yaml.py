...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="500",
        help="Number of runs \t Default: 500",
    )
    parser.add_argument(
        "-f",
        "--fault",
        ...
        type=str,
        default="1",
        help="FI_MAX_MULTIPLE \t Default: 1",
    )
    parser.add_argument(
        "--random_layer",
        action="store_true",
        default=False,
        help="Do you want to randomly select a layer? \t Default: False",
    )
    parser.add_argument(
        "--model_path",
        action="store",
        default="None",
        help="Path to Model.onnx file. It 's required in the case of random layer selection' \t Default: None",
    )

    args = parser.parse_args()
    content = content.replace("numOfRuns: 500", "numOfRuns: " + str(args.runs))
    content = content.replace(
        "fi_max_multiple: 1", "fi_max_multiple: " + str(args.fault)
    )
    ...
    if args.random_layer:
        ...
        content = content.replace(
            "layerId=1986948931", "layerId=" + str(getONNXId(random_element_key))
        )
