...
class NLPModel:
    ...
    def calculateEmbeddingDistanceCodeBert(self):
        ...
        for key in self.fi_outputs.keys():
            if (
                self.fi_outputs[key] != self.model_correct_pred
                and key in self.fi_runs.keys()
            ):
                ...
                distance = cos(
                    correct_embeddings[0][0], incorrect_embeddings[0][0]
                ).item()
    ...
    def analyzeFaults(self):
        ...
        if len(self.fi_runs.keys()) != len(self.fi_outputs.keys()):
            print("----------------------------------")
            print(
                "WARNING: Mismatch between number of FI runs and number of FI outputs"
            )
            k = [i for i in self.fi_outputs.keys() if i not in self.fi_runs.keys()]
            print("\t Keys not found: " + str(k))
            print(
                "\t Prediction corresponding to these keys: "
                + str([self.fi_outputs[i] for i in k])
            )
        ...
        self.confidence_scores_95 = 1.96 * math.sqrt(
            self.sdc_rate * (1 - self.sdc_rate) / self.fi_total_runs
        )
        self.confidence_scores_99 = 2.58 * math.sqrt(
            self.sdc_rate * (1 - self.sdc_rate) / self.fi_total_runs
        )

        # Calculate Average Cosine Similarity and 95% confidence scores
        distances = [
            self.fi_outputs_embedding_distance[key]
            for key in self.fi_outputs_embedding_distance.keys()
        ]
        self.avg_cosine_similarity, self.CI_cosine_similarity = (
            self.mean_confidence_interval(distances, confidence=0.95)
        )
    ...
    def printStats(self, args):
        if args.sdc_rates:
            ...
            print(
                "Total SDCs: "
                + str(len(self.all_sdc))
                + "  Total runs: "
                + str(self.fi_total_runs)
            )

        if args.calc_cs:
            print("Average Cosine Similarity: " + str(self.avg_cosine_similarity))
            print(
                "Cosine Similarity Confidence Scores: 95%: "
                + str(self.CI_cosine_similarity)
            )
        ...
        if args.verbose:
            ...
            print(
                "All SDCs: "
                + str([[key, self.fi_outputs[key]] for key in self.all_sdc])
            )
            print("Correct prediction: " + str(self.model_correct_pred))
            ...
            print(
                "Corrupted vals: "
                + str([[key, self.fi_runs[key].fi_new_val] for key in self.all_sdc])
            )
            print(
                "FI Layers: "
                + str([[key, self.fi_runs[key].fi_layer] for key in self.all_sdc])
            )
...
def main():
    ...
    parser.add_argument(
        "model_name",
        help="Name of the model. Can be one of: biobert, codebert, gpt2, t5-encoder, t5-decoder, roberta",
    )
    parser.add_argument(
        "--calculate-embeddings",
        action="store_true",
        default=False,
        help="Calculate embedding distance between correct and incorrect predictions. \t Default: False",
    )
    parser.add_argument(
        "--sdc-rates",
        action="store_true",
        default=False,
        help="Print the SDC rate \t Default: False",
    )
    parser.add_argument(
        "--calc-cs",
        action="store_true",
        default=False,
        help="Print the Cosine Similarity rate \t Default: False",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    for dir in [f for f in os.scandir(args.results) if f.is_dir()]:
        ...
        NLPModelObj = NLPModel(
            args.model_name, input_num, fi_stats_dir, model_output_dir
        )
    ...
    if args.export_csv:
        ...
        header = [
            "Input",
            "SDC Rate",
            "SDC Rate Confidence Scores",
            "Total SDCs",
            "Total runs",
            "Average Cosine Similarity",
            "Cosine Similarity Confidence Scores",
        ]
        rows = []
        for i in sorted(stats_for_inputs.keys()):
            rows.append(
                [
                    i,
                    stats_for_inputs[i].sdc_rate,
                    stats_for_inputs[i].confidence_scores_95,
                    len(stats_for_inputs[i].all_sdc),
                    stats_for_inputs[i].fi_total_runs,
                    stats_for_inputs[i].avg_cosine_similarity,
                    stats_for_inputs[i].CI_cosine_similarity,
                ]
            )
            ...
            for sdc in stats_for_inputs[i].all_sdc:
                ...
                rows.append(
                    [
                        ...
                        wrong_pred,
                        "",
                        stats_for_inputs[i].model_correct_pred,
                        "",
                        fi_run,
                        fi_bit,
                        fi_layer,
