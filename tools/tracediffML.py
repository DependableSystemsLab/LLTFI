...
#traceDiff.py
#  Author: Sam Coulter, Abraham Chan (modified for ML)
...
def traceDiff(argv, output = 0):
  ...
  faultyTraceLines = faultyTraceLines[:maxDebugTrace+1]

  goldFileName = argv[1]

  #Remove traces from golden trace that happened before fault injection point
  faultyTraceEndPoint = faultyTraceStartPoint + maxDebugTrace
  tmpGoldFileName = "temp.stat.trace.prof.txt"
  cmd_str = "sed -n '" + str(faultyTraceStartPoint) + "," + str(faultyTraceEndPoint-1) + "p;" + str(faultyTraceEndPoint) + "q' " + goldFileName + " > " + tmpGoldFileName
  os.system(cmd_str)

  #Read condensed golden trace file
  goldFile = open(tmpGoldFileName, 'r')
  ...
  #delete temp file for condensed golden trace
  cmd_str = "rm " + tmpGoldFileName
  os.system(cmd_str)
