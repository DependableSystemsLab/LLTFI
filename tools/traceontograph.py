        ...
        for i in range(len(graphLines)):
            ...
            for s, e in affectedEdges:
                summation = sum(
                    ...
                    if ("llfiID_" + str(s) + " -> " + "llfiID_" in line)
                    and "blue" not in line
                )
                if summation == 2:
                    if (
                        "llfiID_" + str(s) + " -> " + "llfiID_" + str(e) + ";"
                    ) in graphLines[i]:
...
if __name__ == "__main__":
    if len(sys.argv) >= 2 and (sys.argv[1] == "-h" or sys.argv[1] == "--help"):
        print(
            (
                "%(prog)s applies the program trace difference summary to program static control-data-flow graph to visualize it\n\n"
                "running option: %(prog)s <trace difference report> <program dot-formatted CDFG>"
                % {"prog": prog}
            ),
            file=sys.stderr,
        )
        ...
        print(
            "Error: running option: %(prog)s <trace difference report> <program dot-formatted CDFG>"
            % {"prog": prog},
            file=sys.stderr,
        )
        sys.exit(1)
