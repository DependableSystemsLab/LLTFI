...
class diffBlock:
    def __init__(self, lines):
        ...
        debug(lines)
        origHeader, newHeader = (
            lines[0].replace("@", " ").replace("+", " ").replace("-", " ").split()
        )
                ...
                if g.ID == f.ID:
                    if instance.type != 1:
                        ...
                        instance = diffInstance(
                            1,
                            findAdjustedPosition(origStart, goldenRemovedCount),
                            findAdjustedPosition(newStart, faultyRemovedCount),
                            i,
                        )
                    instance.add(
                        "Data Diff: ID: "
...
class ctrlDiffBlock(diffBlock):
    def getRange(self):
        debug(
            "Printing ctrlDiffBlock Range"
        )  # String has to be rturned this way for split to not fail later
        debug(
            str(self.origStart)
            + " "
            + str(self.origLength)
        ...
        return self.origStart, self.origLength, self.newStart, self.newLength
        ...
        for i, (g, f) in enumerate(izip):
            if g and f:
                if instance.type != 2:
                    if instance.summary != None:
                        instanceList.append(
                            instance.summary(self.preDiff, self.postDiff)
                        )
...
class diffReport:
    def __init__(self, goldenLines, faultyLines, startPoint, injectedID):
        ...
        ctrldiff = list(
            difflib.unified_diff(goldenIDs[:], faultyIDs[:], n=1, lineterm="")
        )
        ...
        datadiff = list(
            difflib.unified_diff(goldenLines, faultyLines, n=0, lineterm="")
        )
        ...
        while i + 1 < len(self.diffs):
            if "Diff@" in self.diffs[i] and "Pre  Diff" in self.diffs[i + 1]:
                ...
                if csplit[5] != "None":
                    ...
                    d = (
                        i + 2
                    )  # Adjusting so we dont check the find the pre diff of the diff@ instance we
                    # are currently on.
                    while d < len(self.diffs):
                        if "Post Diff" in self.diffs[d]:
                            ...
                            d = len(
                                self.diffs
                            )  # If we found a new ctrl diff block before finding a post diff,
                            # exit the loop early
                        d += 1
